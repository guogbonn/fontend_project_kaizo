...
function task_elm_dom_obj(
                        ...
                        )
  {
    ...
    if(!options["taskview"]){
      var delete_task_button = `<img onclick="show_modal({task_delete :'${task_id}',delete:true})" height="20" width="20" src="img/ProjectMap/delete.svg">`
      var edit_task_button = ` <img class="task_edit_button" onclick="show_modal({ edit_task: '${task_id}'})" height="20" width="20" src="img/ProjectMap/edit.svg">`
    }else{
      var delete_task_button = ""
      var edit_task_button = ""
    }
    console.log("task_unique_id",task_unique_id[task_id])
  var task_elm = `<div id="${task_id}_card" class="task_item">

              <div class="task_container">

                <div class="task_menu">
                  ${delete_task_button}
                  <div class="add_story" onclick="show_modal({ add_story:true, story_task_id: '${task_id}'})"><p>+</p></div>
                  ${edit_task_button}
                </div>
              ...
              <div id="phase_container_story_toggle${task_id}" onclick="toggle_story_container('${task_id}')" class="task_story_arrow">
                <div id="${task_id}_arrow" class="task_arrow_up"></div>
              </div>
            </div>
            <div id="${task_id}_story_container" class="story_container">${create_stories(stories,task_id)} </div>
    `
...
function create_phase_task_ui(phase_id) {
  ...
  if(phase_id == null){ //possible on deletion of phase
    ...
    phase_container.innerHTML = `<img onclick="expand_card('phase_card',this)" height="20" width="20" src="img/ProjectMap/expand.svg"><div style="margin: auto;height: 50%;font-size: x-large;"><h3> No Phase Selected</h3></div>`
    return;
  }
  // show phase menu
...
function create_task_view_ui(on = false) {
  ...
  task_story_dropdown = [];// refresh on any taskview or phaseview rerender, force rerender
  story_elm = []; // contains the task id,  not story elem, used to controll toggle containers
  current_task_view_diplay_person_id = null; // tasks are populated of the person that is clicked and rerendered on this as well
  ...
  for(const  person in people_unique_id){
    if(people_unique_id[person]["project_id"] == current_project_id && people_unique_id[person]["deleted"] == "false"){
      var member_elm = `
      <div class="team_member_task_option task_view_person_button" person_id="${person}">
...
var current_task_view_diplay_person_id = null;
function populate_tasks_for_person(person_id) {
  current_task_view_diplay_person_id = person_id;
  document.getElementById('phase_card_phase_menu').style.visibility = "visible";
  var tasks_container = ""
...
// select the phase group list
  ...
  for (var i = 0; i < map.length; i++) {
    ...
    console.log("get_phase_group_date_range",map[i])
    ...
    if(node.id()in phase_unique_id){
      // the phase id has associatiated tasks
        if( phase_unique_id[node.id()]["type"] == "real"){
          ...
          //if on task view change to phase view
          if(task_view == 1){
            toggle_phase_view();
          }
...
function close_error_modal_id(){
  var modal_error_alert_id = "modal_error_alert";
  document.getElementById(modal_error_alert_id).style.visibility = "hidden";
...
function create_sub_phase_elm(project_id){
  var subphase_res = ""
  for(var prop in project_schemes[project_id]){
    if (prop != "begin"){
...
function change_project_ui(current_proj_id) {
  ...
  current_sub_phase = null; // sub view sub phase
  ...
  for(const people in people_unique_id ){
    ...
    if(people_unique_id[people]["server_id"] == user_server_id && people_unique_id[people]["deleted"] == "false" && people_unique_id[people]["project_id"] == current_project_id){
      current_user = people;
      break;
    }
  }

    remove_ability_if_not_project_owner();
...
function remove_ability_if_not_project_owner() {
  // hide
  // create project
  // people
  // team
  if(current_project_id != null && user_server_id != project_unique_id[current_project_id]["project_owner"]){
    var elements = document.getElementsByClassName('add_project')
    var ele_content = document.getElementsByClassName('add_project_content')
    for(var i = 0; i < elements.length ; i++){
      elements[i].style.visibility = "hidden";
      ele_content[i].removeAttribute("onclick");
    }
  }else{
    // show when project owner
    var elements = document.getElementsByClassName('add_project')
    var ele_content = document.getElementsByClassName('add_project_content')
    var arr = ["show_modal({create_project: true})","show_modal({team_create:true})","show_modal({people_create:true})"]
    for(var i = 0; i < elements.length ; i++){
      elements[i].style.visibility = "visible";
      ele_content[i].removeAttribute("onclick");
      ele_content[i].setAttribute('onclick',arr[i])
    }
  }
...
function map_message_ui(message,hide_seconds = null) {
  //user_diplay_message
  console.log("show user message")
  var message_elm = document.getElementById('user_diplay_message')
  message_elm.style.visibility = "visible";
  document.getElementById('user_diplay_message').style.visibility = "visible"
  message_elm.innerHTML = message;


}
function hide_user_message_ui(){
  document.getElementById('user_diplay_message').style.visibility = "hidden";
}
